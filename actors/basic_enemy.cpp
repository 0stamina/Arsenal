...
{
    if(Vector2Distance(actor_list[0].position, actor->position) >= 405)
    {
        //Vector2 dir =Vector2Normalize(Vector2Subtract(actor_list[0].position, actor->position)
        actor->position = Vector2Add(actor_list[0].position, Vector2Scale(actor->move_dir, 400));
    }
